...
def lambda_handler(event, context):
    ...
    for instance in rds_instances:
        ...
        # Check if the engine type is MSSQL
        if engine.lower() in sql_postgres_engines:
            ...
            # Paginate results until "rds.force_ssl" parameter is found
            ...
            for page in paginator.paginate(DBParameterGroupName=db_parameter_group):
                ...
                # Find the 	rds.force_ssl parameter value
                for parameter in page['Parameters']:
                    if parameter['ParameterName'] == 'rds.force_ssl':
                        require_secure_transport=parameter.get('ParameterValue')
                        break

            # Determine compliance based on rds.force_ssl value
            if require_secure_transport == '1':
                compliance_type='COMPLIANT'
            else:
                compliance_type='NON_COMPLIANT'
        else:
            # If the engine type is not MSSQL, mark as non-applicable
